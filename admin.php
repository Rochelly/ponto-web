...
for ($i = 1; $i < 13; $i++) {
    if ($i == $mes) echo "<option value='{$i}' selected='selected'>{$meses[$i]}</option>";
...
for ($i = 2015; $i <= $ano_atual; $i++) {
    if ($i == $ano_selecionado) echo "<option value='{$i}' selected='selected'>{$i}</option>";
...
for ($i = 0; $i < count($chefiaDepartamento); $i++) {
    ...
    $departamentosChefes[$chefiaDepartamento[$i]->departamento_id] = $chefiaDepartamento[$i]->nome;
}
?>
                <div class="row">
                    <div class="col-md-4"></div>
                    <div class="col-md-3">
                        <select id="departamento" name="departamento" class="form-control" onchange="this.form.submit()">
                            <?php
for ($i = 0; $i < count($chefiaDepartamento); $i++) {
    echo "<option value='{$chefiaDepartamento[$i]->departamento_id}'" . (($chefiaDepartamento[$i]->departamento_id == $departamentoSelecionado) ? " selected = 'selected'" : "") . ">{$chefiaDepartamento[$i]->descricao}</option>";
}
?>
                        </select>
                    </div>
...
for ($j = 0; $j < count($funcionarios); $j++) {
    ...
    $data_admissao = $funcionarios[$j]->data_admissao;
    
    //recuperando a data de admissao do  funcionario
    $data_admissaoVet = explode('/', $data_admissao);
    
    // data fragmentada pra se  retirar as partes da data
    $data_admissao = str_replace('/', '-', $data_admissao);
    
    if ($funcionarios[$j]->data_demissao != null) {
        $data_demissao = $funcionarios[$j]->data_demissao;
        
        //recuperando a data de demissao do  funcionario
        $data_demissaoVet = explode('/', $data_demissao);
        ...
        if ($mesFormatado == $data_demissaoVet[1] && $ano_selecionado == $data_demissaoVet[0]) {
            
            //se  o mes de demissao for o  mesmo do  boletim a ser gerado
            $diasDescontadosPorAdmissaoOuDemissao = $diasMes - $data_demissaoVet[2];
        }
    }
    
    if (strtotime($data_admissao) > strtotime($ano_selecionado . '-' . $mesFormatado . '-' . $diasMes)) continue;
    
    // verifica  se e cargo  de  direçao
    $chefiaCD1_CD2 = $ponto->estudanteBool($funcionarios[$j]->n_folha, 2);
    if ($chefiaCD1_CD2 != null and ($chefiaCD1_CD2->resposta[0] == 's' | $chefiaCD1_CD2->resposta[0] == 'S')) continue;
    
    if ($mesFormatado == $data_admissaoVet[1] && $ano_selecionado == $data_admissaoVet[0]) {
